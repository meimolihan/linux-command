compose_install_dpanel
===

全自动、交互式一键部署 DPanel Docker 管理面板，全程无需手动配置环境与文件。

## 一键脚本

```bash
bash <(curl -sL gitee.com/meimolihan/linux-command_sh/raw/master/compose_install_dpanel.sh) 8807 /vol1/1000/compose/dpanel
```

## 项目简介

**[DPanel](https://dpanel.cc/ "DPanel 官方网站")** 是一个轻量级的 Docker 容器管理面板，提供直观的 Web 界面来管理 Docker 容器。通过 Docker 部署，您可以轻松搭建一个功能完整、稳定可靠的容器管理界面，支持实时监控、容器管理和资源统计。

  - **🌐 `DPanel` 官网地址：[https://dpanel.cc/](https://dpanel.cc/ "DPanel 官网")**
  - **🐱 `GitHub` 项目地址：[https://github.com/donknap/dpanel](https://github.com/donknap/dpanel "GitHub 项目地址")**
  - **🐋 `Docker` 镜像地址：[https://hub.docker.com/r/dpanel/dpanel](https://hub.docker.com/r/dpanel/dpanel "Docker 镜像地址")**

## 效果预览

![执行脚本效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_dpanel-01.webp "截图演示")

![WEB效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_dpanel-02.webp "截图演示")

## 补充说明

该脚本用于一键部署 DPanel Docker 管理面板，基于 Docker Compose 实现，适合需要可视化管理和监控 Docker 容器的场景。
...
| 映射端口 | 显示面板访问端口（默认 8807） |
| 端口状态 | 检查并开放防火墙端口 |
| 容器清理 | 显示旧容器（dpanel）和镜像的清理结果 |
| 配置文件 | 显示 docker-compose.yml 创建状态 |
| 容器启动 | 显示容器启动结果 |
| 容器状态 | 显示容器 ID、名称、状态、端口等信息 |
| 访问地址 | 显示 DPanel 的 HTTP 访问 URL |

### 注意事项

* 脚本需要 root 权限或 sudo 权限来安装依赖和配置防火墙
* 默认使用 dpanel/dpanel:latest 镜像
* 部署目录默认为 /vol1/1000/compose/dpanel，可通过参数修改
* 默认端口为 8807，确保端口未被占用且防火墙已开放
* 面板需要挂载 /var/run/docker.sock 来管理宿主机容器
* 数据持久化存储在部署目录下的 ./dpanel 目录
...
> 2. 先目录后端口 
>
>   > 脚本.sh /vol1/1000/compose/dpanel 8807 
> 3. 先端口后目录 
>
>   > 脚本.sh 8807 /vol1/1000/compose/dpanel 
> 4. 只传目录 
>
>   > 脚本.sh /vol1/1000/compose/dpanel 
> 5. 只传端口 
>
>   > 脚本.sh 8807
...
docker rm -f dpanel && docker rmi -f dpanel/dpanel:latest && rm -rf /vol1/1000/compose/dpanel
```
...
- [compose_install_dpanel](../c/compose_install_dpanel.html "Dpanel")  👈 当前所在位置
