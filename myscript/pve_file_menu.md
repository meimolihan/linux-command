...
该脚本是 PVE（Proxmox VE）系统下的交互式文件管理器，专注于镜像文件管理和备份文件管理，适合在 PVE 环境下管理 ISO 镜像、备份文件的场景。

### 功能特点

* PVE 专用：针对 PVE 系统优化，识别 PVE 目录结构
* 镜像管理：浏览、上传、删除 ISO 镜像文件
* 备份管理：查看、恢复、删除虚拟机备份
* 快捷操作：内置 PVE 常用目录快捷入口
...
| 功能分类 | 包含操作 |
| --- | --- |
| ISO 镜像管理 | 浏览镜像、上传镜像、删除镜像 |
| 备份文件管理 | 查看备份、恢复备份、删除备份 |
| 目录操作 | 浏览目录、创建目录、删除目录 |
| 文件操作 | 复制、移动、删除、重命名 |
...
# 传入工作目录路径
bash <(curl -sL gitee.com/meimolihan/linux-command_sh/raw/master/pve_file_menu.sh) /var/lib/vz/template/iso
```
...
* 需要运行在 PVE 系统上
* 删除操作无法恢复，请谨慎操作
* ISO 镜像默认存储在 /var/lib/vz/template/iso
* 备份文件默认存储在 /var/lib/vz/dump
