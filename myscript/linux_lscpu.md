...
```bash
bash <(curl -sL gitee.com/meimolihan/linux-command_sh/raw/master/linux_lscpu.sh)
```

## 效果预览

![执行脚本效果预览](https://file.meimolihan.eu.org/screenshot/linux_lscpu.webp "截图演示")

## 补充说明

该脚本用于美化显示 CPU 架构信息，基于 `lscpu` 命令实现，适合快速查看 CPU 型号、核心数、架构等信息的场景。

### 功能特点

* 彩色输出：字段名（青色）、值（绿色）使用不同颜色区分
* 完整信息：显示架构、CPU 型号、核心数、线程数、缓存等完整信息
* 表格格式化：自动使用 `column` 命令对齐输出
* 排序显示：按字段重要性排序，便于查找关键信息
