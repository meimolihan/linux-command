compose_install_it-tools
===

自动检查并安装 Docker 环境、清理冲突容器镜像、支持自定义目录 / 端口、一键部署 IT 工具箱并显示访问地址的自动化部署脚本。

## 一键脚本

```bash
bash <(curl -sL gitee.com/meimolihan/linux-command_sh/raw/master/compose_install_ittools.sh) 8808 /vol1/1000/compose/it-tools
```

## 项目简介

**[IT-Tools](https://it-tools.tech/ "IT-Tools 在线工具")** 是一个集成了多种实用开发工具的网络应用，为开发者和IT专业人员提供了一站式的工具集合。通过 Docker 部署，您可以轻松访问各种编码、转换、计算和网络工具，无需安装多个独立应用程序。

  - **🌐 `IT-Tools` 官网地址：[https://it-tools.tech/](https://it-tools.tech/ "IT-Tools 在线工具")**
  - **🐱 `GitHub` 项目地址：[https://github.com/CorentinTh/it-tools](https://github.com/CorentinTh/it-tools "IT-Tools 工具集合")**
  - **🐋 `Docker` 镜像地址：[https://hub.docker.com/r/qingfeng2336/it-tools](https://hub.docker.com/r/qingfeng2336/it-tools "Docker Hub 镜像地址")**

## 效果预览

![执行脚本效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_ittools-01.webp "截图演示")

![WEB效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_ittools-02.webp "截图演示")

## 补充说明

该脚本用于一键部署 IT-Tools 工具箱，基于 Docker Compose 实现，适合开发者和 IT 专业人员快速搭建集成式工具集合服务。
...
* 支持自定义时区：默认设置为 Asia/Shanghai
* 部署后显示容器状态和访问地址
* 自动清理：容器配置为 always 重启策略
...
| 映射端口 | 显示服务访问端口（默认 8088） |
| 端口状态 | 检查并开放防火墙端口 |
| 容器清理 | 显示旧容器（it-tools）和镜像的清理结果 |
| 配置文件 | 显示 docker-compose.yml 创建状态 |
| 容器启动 | 显示容器启动结果 |
| 容器状态 | 显示容器 ID、名称、状态、端口等信息 |
| 访问地址 | 显示 IT-Tools 的 HTTP 访问 URL |

### 注意事项

* 脚本需要 root 权限或 sudo 权限来安装依赖和配置防火墙
* 默认使用 corentinth/it-tools:latest 镜像
* 部署目录默认为 /vol1/1000/compose/it-tools，可通过参数修改
* 默认端口为 8088，确保端口未被占用且防火墙已开放
* 容器以 root 用户（PUID=0, PGID=0）运行
* 时区可通过修改 TZ 环境变量调整
...
> 2. 先目录后端口 
>
>   > 脚本.sh /vol1/1000/compose/it-tools 8088 
> 3. 先端口后目录 
>
>   > 脚本.sh 8088 /vol1/1000/compose/it-tools
> 4. 只传目录 
>
>   > 脚本.sh /vol1/1000/compose/it-tools
> 5. 只传端口 
>
>   > 脚本.sh 8088
...
docker rm -f it-tools && docker rmi -f corentinth/it-tools:latest && rm -rf /vol1/1000/compose/it-tools
```
...
- [compose_install_it-tools](../c/compose_install_it-tools.html "IT-Tools 工具箱")  👈 当前所在位置
