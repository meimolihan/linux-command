...
从零开始完整安装配置 WSL2 Ubuntu 系统的详细流程，涵盖环境准备、系统安装、基础配置、软件部署以及 Docker 环境搭建。

## 补充说明

该教程提供从零开始完整安装配置 WSL2 Ubuntu 系统的详细流程，涵盖环境准备、系统安装、基础配置、软件部署以及 Docker 环境搭建，适合在 Windows 上使用 Ubuntu 子系统的场景。
...
| 安装 Ubuntu | 查看可用发行版、安装 Ubuntu、初始化配置 |
| 基础配置 | 换源、安装常用工具、配置时区 |
...
- `--name ubuntu`：设置实例名称为 ubuntu，便于管理
...
```bash
sudo apt install -y curl wget git vim nano zip unzip bash-completion locales
```
...
参考 [wsl_ubuntu_autostart](./c/wsl_ubuntu_autostart.html) 脚本，实现 Windows 开机自动启动 WSL 和 Docker 服务。
...
# 导出 Ubuntu 系统
wsl --export ubuntu C:\Users\Administrator\wsl-ubuntu-backup.tar
```
...
```powershell
# 导入为新实例
wsl --import ubuntu-new C:\Users\Administrator\wsl-ubuntu C:\Users\Administrator\wsl-ubuntu-backup.tar --version 2
...
```powershell
diskpart
select vdisk file="C:\Users\Administrator\AppData\Local\wsl\ubuntu\ext4.vhdx"
...
- [wsl_install_ubuntu](../c/wsl_install_ubuntu.html "WSL Ubuntu 安装配置指南")  👈 当前所在位置
...
- [Ubuntu 官方文档](https://ubuntu.com/ "Ubuntu 官方文档")
- [Docker 官方安装指南](https://docs.docker.com/engine/install/ubuntu/ "Docker 官方安装指南")