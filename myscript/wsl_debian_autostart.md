wsl_debian_autostart
===

通过在 Windows 启动目录放置自启脚本，实现开机自动唤醒 WSL Debian 并后台启动 Docker 服务，全程静默运行、日志留存，高效完成 WSL 环境常驻部署。

## 补充说明

该脚本用于在 Windows 启动目录放置自启脚本，实现开机自动唤醒 WSL Debian 并后台启动 Docker 服务，适合需要 WSL 环境常驻运行的场景。

### 功能特点

* 开机自启：通过 Windows 启动目录实现 WSL Debian 开机自动启动
...
# 创建自启动脚本（在 CMD 中执行）
copy nul "%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup\WSL_Debian_自启.bat" /y && notepad "%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup\WSL_Debian_自启.bat"
```

### 注意事项

* 需要 Windows 10 2004+ 或 Windows 11
* 需要先安装并配置好 WSL Debian
* 需要 Debian 中已安装并配置 Docker
* 日志文件默认保存在桌面，可自定义路径
* 建议先手动测试脚本，确认无误后再添加到启动目录

## 列出所有已安装的 Linux 发行版（PowerShell/CMD）命令

- 下面脚本启动的是 **debian** 按需修改
...
- **Windows 开机自启 WSL Debian 并自动启动 Docker**的优化脚本，自带**UTF-8 不乱码日志、自动清理日志大小、中文友好、运行后自动打开日志查看结果**。
...
@echo off
chcp 65001 >nul 2>&1
set "logfile=%USERPROFILE%\Desktop\WSL_Autostart.log"
...
echo 正在启动WSL Debian ... >> "%logfile%"

wsl -d debian -- echo "WSL Debian 唤醒成功" >> "%logfile%" 2>&1

if %errorlevel% equ 0 (
    echo WSL Debian 启动成功 >> "%logfile%"
    wsl -d debian -- /usr/sbin/service docker start >> "%logfile%" 2>&1
    echo Docker 服务已启动 >> "%logfile%"
) else (
    echo WSL Debian 启动失败 >> "%logfile%"
)
...
start powershell -NoExit -Command "cd ~/Desktop; wsl.exe -d debian"
```
...
# 手动打开日志文件（CMD）命令
notepad "%USERPROFILE%\Desktop\WSL_Autostart.log"
...
wsl --set-default debian && wsl -l -v

# 立即关闭所有正在运行的 WSL 发行版（包括 Debian、Ubuntu 等），完全退出 WSL 虚拟机
...
- [wsl_debian_autostart](../c/wsl_debian_autostart.html "WSL Debian 开机自启脚本")  👈 当前所在位置
