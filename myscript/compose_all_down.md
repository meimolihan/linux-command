...
交互式批量停止 Docker Compose 项目的管理脚本。

## 一键脚本

```bash
bash <(curl -sL gitee.com/meimolihan/linux-command_sh/raw/master/compose_all_down.sh) /vol1/1000/compose
```

## 效果预览

![](https://file.meimolihan.eu.org/screenshot/compose_all_down.webp)

## 补充说明

该脚本用于交互式批量停止指定目录下的所有 Docker Compose 项目，基于 docker-compose/docker compose 命令实现，适合需要一键停止多个容器项目的场景。

### 功能特点

* 支持传参和交互式两种模式：可直接指定目录或交互选择
* 智能扫描项目：自动查找包含 docker-compose.yml/yaml 的目录
* 预设目录推荐：内置常用 Compose 目录路径并标记推荐项
* 运行时项目检测：通过 Docker API 获取运行中的项目目录
* 二次确认机制：防止误操作，需用户确认后才执行停止
* 支持自动确认模式：使用 -y 参数可跳过交互确认
* 详细统计信息：显示总计项目数、成功停止数
...
| 扫描到的项目 | 列出运行中的 Compose 项目目录 |
| 推荐的工作目录 | 标记有运行中项目的预设目录 |
| 项目状态 | 显示每个项目的停止结果（✓ 已停止） |
| 停止完成统计 | 总计项目数、成功停止数 |
...
* 交互模式下会扫描运行中的容器来推荐工作目录
* 使用 -y/--yes 参数可跳过确认提示，适合自动化脚本
* 如果目录不存在或没有 Compose 文件，脚本会给出警告并退出
