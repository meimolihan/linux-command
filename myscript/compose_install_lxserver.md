compose_install_lxserver
===

一键部署 落雪音乐 lxserver 服务，支持自定义部署路径、端口及密码，自动清理旧容器并生成 docker-compose 配置。
...
```bash
bash <(curl -sL gitee.com/meimolihan/linux-command_sh/raw/master/compose_install_lxserver.sh) 9526 /vol1/1000/compose/lxserver
```

## 项目简介

  - **🌐 `LX Sync Server` 官网地址：[https://xcq0607.github.io/lxserver/](https://xcq0607.github.io/lxserver/ "LXServer 文档")**
  - **🐱 `GitHub` 项目地址：[https://github.com/XCQ0607/lxserver](https://github.com/XCQ0607/lxserver "GitHub 代码托管平台")**
  - **🐋 `Docker` 镜像地址：[https://hub.docker.com/r/xcq0607/lxserver](https://hub.docker.com/r/xcq0607/lxserver "Docker Hub 镜像地址")**

## 效果预览

![执行脚本效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_lxserver-01.webp "截图演示")

![WEB效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_lxserver-02.webp "截图演示")

## 补充说明

该脚本用于一键部署洛雪音乐 LX Server 服务，基于 Docker Compose 实现，适合需要搭建私有音乐服务器的场景。

### 功能特点

* 支持交互式和传参两种部署模式（可指定目录、端口、密码）
* 自动检查并安装 Docker 和 Docker Compose 环境
* 智能端口管理：检查端口是否放行，自动开放并持久化 iptables 规则
* 清理旧容器：部署前自动清理 lxserver、lx-sync-server 和相关镜像
* 自动生成 docker-compose.yml 配置文件
* 支持自定义 Web 管理密码和播放器密码
* 默认关闭播放器密码验证（ENABLE_WEBPLAYER_AUTH=false）
* 提供多个在线音源：野花音源、野草音源、聚合 API 接口
* 数据持久化：配置、日志、缓存分别挂载存储
* 部署后显示容器状态和访问地址
...
| 部署目录 | 显示 Compose 文件存放路径（默认 /vol1/1000/compose/lxserver） |
| 映射端口 | 显示服务访问端口（默认 9526） |
| 端口状态 | 检查并开放防火墙端口 |
| Web 管理密码 | 显示设置的管理密码 |
| 播放器密码 | 显示设置的播放器密码 |
| 容器清理 | 显示旧容器（lxserver、lx-sync-server）和镜像的清理结果 |
| 配置文件 | 显示 docker-compose.yml 创建状态 |
| 容器启动 | 显示容器启动结果 |
| 容器状态 | 显示容器 ID、名称、状态、端口等信息 |
| 访问地址 | 显示洛雪音乐的 HTTP 访问 URL |

### 注意事项

* 脚本需要 root 权限或 sudo 权限来安装依赖和配置防火墙
* 默认使用 ghcr.io/xcq0607/lxserver:latest 镜像
* 部署目录默认为 /vol1/1000/compose/lxserver，可通过参数修改
* 默认端口为 9526，映射容器内的 9527 端口
* Web 管理密码和播放器密码可自定义，默认为相同密码
* 音源文件可从 https://download.meimolihan.eu.org/lxserver/ 获取
* 数据持久化存储在部署目录下的 ./data、./logs、./cache 目录
...
```bash
https://download.meimolihan.eu.org/lxserver/野花音源.js
```

```bash
https://download.meimolihan.eu.org/lxserver/野草音源.js
```

```bash
https://download.meimolihan.eu.org/lxserver/聚合API接口.js
```

```bash
https://download.meimolihan.eu.org/lxserver/lx-music-source.js脚本源码
```
...
> 2. 先目录后端口 
>
>   > 脚本.sh /vol1/1000/compose/lxserver 9526
> 3. 先端口后目录 
>
>   > 脚本.sh 5003 /vol1/1000/compose/lxserver 
> 4. 只传目录 
>
>   > 脚本.sh /vol1/1000/compose/lxserver
> 5. 只传端口 
>
>   > 脚本.sh 9526
...
docker rm -f lxserver && docker rmi -f ghcr.io/xcq0607/lxserver:latest && rm -rf /vol1/1000/compose/lxserver
```
...
- [compose_install_lxserver](../c/compose_install_lxserver.html "LX-Server 洛雪音乐")  👈 当前所在位置
