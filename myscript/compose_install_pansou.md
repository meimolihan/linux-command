...
```bash
bash <(curl -sL gitee.com/meimolihan/linux-command_sh/raw/master/compose_install_pansou.sh) 8110 /vol1/1000/compose/pansou
```
...
**Pansou** 是一个高性能的网盘资源搜索 API 服务，支持 TG 搜索和自定义插件搜索。通过 Docker 部署，可以快速搭建个人或团队使用的文件搜索服务。🌟
  ...
  - **🐱 `GitHub` 项目地址：[https://github.com/fish2018/pansou](https://github.com/fish2018/pansou "GitHub 代码托管平台")**

## 效果预览

![执行脚本效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_pansou-01.webp "截图演示")

![WEB效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_pansou-02.webp "截图演示")

## 补充说明

该脚本用于一键部署 PanSou 私人网盘搜索引擎，基于 Docker Compose 实现，适合需要搭建私有网盘资源搜索服务的场景。
...
* 清理旧容器：部署前自动清理 pansou 容器和相关镜像
* 自动生成 docker-compose.yml 配置文件
* 使用 bridge 网络模式：容器独立网络栈
* 数据持久化：配置数据存储在 ./data 目录（/app/data）
* 支持大量搜索插件：混合盘、即刻盘、盘维基、盘搜索、盘他、曲盘搜、素素、海盗湾、万能OU、学习指南、盘友圈、芝镇、拉比、木偶、欧哥、闪电、多多、虎斑、二小、喵搜、Fox4K、片库、CL毛、无极、CL地、小张、LibVio、乐静、XB6V、XYS、DDYS、HDMoli、雨花阁、U3C3、JavDB、CL熊、巨头社、SDSO、小吉、小樱花、海搜、比心等
* 支持代理配置：可通过环境变量设置 HTTP/HTTPS 代理
* 部署后显示容器状态和访问地址
...
| 部署目录 | 显示 Compose 文件存放路径（默认 /vol1/1000/compose/pansou） |
| 映射端口 | 显示服务访问端口（默认 8110，映射容器内 80 端口） |
| 端口状态 | 检查并开放防火墙端口 |
| 容器清理 | 显示旧容器（pansou）和镜像的清理结果 |
| 配置文件 | 显示 docker-compose.yml 创建状态 |
| 容器启动 | 显示容器启动结果 |
| 容器状态 | 显示容器 ID、名称、状态、端口等信息 |
| 访问地址 | 显示 PanSou 的 HTTP 访问 URL |

### 注意事项

* 脚本需要 root 权限或 sudo 权限来安装依赖和配置防火墙
* 默认使用 ghcr.io/fish2018/pansou-web:latest 镜像
* 部署目录默认为 /vol1/1000/compose/pansou，可通过参数修改
* 默认端口为 8110，映射容器内的 80 端口
* 数据持久化存储在部署目录下的 ./data 目录
* 支持大量网盘搜索插件，涵盖多种资源类型
* 代理配置按需修改，默认代理地址可能需要根据环境调整
* 环境变量 ENABLED_PLUGINS 配置了 30+ 搜索插件，可按需增减
...
> 2. 先目录后端口 
>
>   > 脚本.sh /vol1/1000/compose/pansou 8807 
> 3. 先端口后目录 
>
>   > 脚本.sh 8110 /vol1/1000/compose/pansou
> 4. 只传目录 
>
>   > 脚本.sh /vol1/1000/compose/pansou
> 5. 只传端口 
>
>   > 脚本.sh 8110
...
docker rm -f pansou && docker rmi -f ghcr.io/fish2018/pansou-web:latest && rm -rf /vol1/1000/compose/pansou
```
...
- [compose_install_pansou](../c/compose_install_pansou.html "PanSou 阿里云盘")  👈 当前所在位置
