compose_install_random-pic-api
===

random-pic-api 轻量自建随机图片接口。

## 一键脚本

```bash
bash <(curl -sL gitee.com/meimolihan/linux-command_sh/raw/master/compose_install_random-pic-api.sh) 8588 /vol1/1000/compose/random-pic-api
```

## 项目简介

**[random-pic-api](https://github.com/Nei-Xin/random-pic-api "GitHub 代码托管平台")** 是一个轻量级的随机壁纸 API 服务，使用 PHP 开发并通过 Docker 容器化部署。该项目能够根据设备类型自动提供横屏或竖屏壁纸，支持 WebP 格式转换，并提供简洁的 RESTful API 接口。

  - **🐱 `GitHub` 项目地址：<https://github.com/Nei-Xin/random-pic-api>**
  - **🐋 `Docker` 镜像地址：<https://hub.docker.com/r/neixin/random-pic-api>**

  - **🐱 `GitHub` 我的项目地址：<https://github.com/meimolihan/random-pic-api>**
  - **🐋 `Docker` 我的镜像地址：<https://hub.docker.com/repository/docker/mobufan/random-pic-api/general>**

## 效果预览

![执行脚本效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_random-pic-api-01.webp "截图演示")

![WEB效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_random-pic-api-02.webp "截图演示")

## 补充说明

该脚本用于一键部署 Random-Pic-API 轻量级随机图片接口服务，基于 Docker Compose 实现，适合需要搭建私有随机壁纸 API 服务的场景。
...
* 清理旧容器：部署前自动清理 random-pic-api 容器和相关镜像
...
| 部署目录 | 显示 Compose 文件存放路径（默认 /vol1/1000/compose/random-pic-api） |
| 映射端口 | 显示服务访问端口（默认 8588，映射容器内 3000 端口） |
| 端口状态 | 检查并开放防火墙端口 |
| 容器清理 | 显示旧容器（random-pic-api）和镜像的清理结果 |
| 配置文件 | 显示 docker-compose.yml 创建状态 |
| 容器启动 | 显示容器启动结果 |
| 容器状态 | 显示容器 ID、名称、状态、端口等信息 |
| 访问地址 | 显示 Random-Pic-API 的 HTTP 访问 URL |

### 注意事项

* 脚本需要 root 权限或 sudo 权限来安装依赖和配置防火墙
* 默认使用 mobufan/random-pic-api:latest 镜像
* 部署目录默认为 /vol1/1000/compose/random-pic-api，可通过参数修改
* 默认端口为 8588，映射容器内的 3000 端口
* 服务会根据设备类型自动返回横屏或竖屏壁纸
* 支持 WebP 格式转换，可减少带宽消耗
...
> 2. 先目录后端口 
>
>   > 脚本.sh /vol1/1000/compose/random-pic-api 8588
> 3. 先端口后目录 
>
>   > 脚本.sh 5000 /vol1/1000/compose/random-pic-api
> 4. 只传目录 
>
>   > 脚本.sh /vol1/1000/compose/random-pic-api
> 5. 只传端口 
>
>   > 脚本.sh 8588
...
docker rm -f random-pic-api && docker rmi -f mobufan/random-pic-api:latest && rm -rf /vol1/1000/compose/random-pic-api
```
...
- [compose_install_random-pic-api](../c/compose_install_random-pic-api.html "Random-Pic-API")  👈 当前所在位置
