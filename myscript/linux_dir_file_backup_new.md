linux_dir_file_backup_new
===

交互式目录与文件备份工具，兼容传参调用与自定义路径，采用原名称打包、无时间戳命名，备份时强制覆盖同名旧压缩文件。

## 一键脚本

```bash
bash <(curl -sL gitee.com/meimolihan/linux-command_sh/raw/master/linux_dir_file_backup_new.sh) /vol1/1000/compose /vol2/1000/file/myfile/compose/downloads
```

## 效果预览

![执行脚本效果预览](https://file.meimolihan.eu.org/screenshot/linux_dir_file_backup_new.webp "截图演示")

## 补充说明

该脚本用于交互式或参数化备份目录和文件，采用原名称打包（无时间戳），备份时强制覆盖同名旧压缩文件，适合快速备份项目或配置文件的场景。

### 功能特点

* 无时间戳命名：备份文件使用原目录名，方便覆盖更新
* 交互式操作：支持传参或交互式输入工作目录和备份保存目录
* 强制覆盖：同名备份文件自动覆盖，无需手动删除
...
bash <(curl -sL gitee.com/meimolihan/linux-command_sh/raw/master/linux_dir_file_backup_new.sh)
...
* 备份文件名不带时间戳，多次备份会覆盖旧文件
* 如需保留历史备份，请手动重命名或改用 linux_dir_file_backup_old.sh
* 备份大目录可能需要较长时间，请耐心等待
* 确保备份保存目录有足够空间
...
> 不传参（交互式）
>
> > ./linux_dir_file_backup_new.sh
>
> 传参模式（2个必传参数）
>
> > ./linux_dir_file_backup_new  [工作目录]  [备份保存目录]
...
sleep_fractional() {
    ...
    if command -v perl >/dev/null/time 2>&1; then perl -e "select(undef, undef, undef, $seconds)"; return 0; fi
...
- [linux_dir_file_backup_new](../c/linux_dir_file_backup_new.html "文件目录备份脚本(无时间戳)")  👈 当前所在位置
