compose_install_xiaomusic
===

xiaomusic 解锁小爱音箱 听歌自由 的 Docker 服务。

## 一键脚本

```bash
bash <(curl -sL gitee.com/meimolihan/linux-command_sh/raw/master/compose_install_xiaomusic.sh) 58090 /vol1/1000/compose/xiaomusic
```

## 项目简介

**[XiaoMusic](https://xdocs.hanxi.cc/)** 是一个开源工具，它允许您通过小爱音箱播放本地音乐文件，并在音乐不存在时自动从网络下载。通过 Docker 部署，您可以轻松地将您的 NAS 或服务器转变为个人音乐流媒体服务中心，让小爱音箱摆脱平台版权限制，畅享个人音乐库。

  - **🌐 `XiaoMusic` 官网地址：[https://xdocs.hanxi.cc/](https://xdocs.hanxi.cc/)**
  - **🐱 `GitHub` 项目地址：[https://github.com/hanxi/xiaomusic](https://github.com/hanxi/xiaomusic "XiaoMusic 小米音乐")**
  - **🐋 `Docker` 镜像地址：[https://hub.docker.com/r/hanxi/xiaomusic](https://hub.docker.com/r/hanxi/xiaomusic "Docker Hub 镜像地址")**

## 效果预览

![执行脚本效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_xiaomusic-01.webp "截图演示")

![WEB效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_xiaomusic-02.webp "截图演示")

## 补充说明

该脚本用于通过 Docker Compose 一键部署 XiaoMusic（解锁小爱音箱听歌自由）服务，适合需要在 NAS 或服务器上搭建个人音乐流媒体服务的场景。

### 功能特点

* 交互式部署：支持参数传入或交互式输入部署目录和端口
* 自动配置：自动生成 docker-compose.yml 配置文件
* 特权模式：支持硬件解码，提升音乐播放性能
* 持久化存储：音乐数据和配置独立存储，便于迁移和备份
* 彩色输出：全程彩色提示，操作状态一目了然
...
| 配置项 | 默认值 |
| --- | --- |
| 部署目录 | /vol1/1000/compose/xiaomusic |
| 访问端口 | 58090 |
| 容器名称 | xiaomusic |
...
```bash
# 交互式安装（会提示输入目录和端口）
bash <(curl -sL gitee.com/meimolihan/linux-command_sh/raw/master/compose_install_xiaomusic.sh)
...
> 2. 先目录后端口 
>
>   > 脚本.sh /vol1/1000/compose/xiaomusic 58090
> 3. 先端口后目录 
>
>   > 脚本.sh 5000 /vol1/1000/compose/xiaomusic
> 4. 只传目录 
>
>   > 脚本.sh /vol1/1000/compose/xiaomusic
> 5. 只传端口 
>
>   > 脚本.sh 58090
...
docker rm -f xiaomusic && docker rmi -f hanxi/xiaomusic && rm -rf /vol1/1000/compose/xiaomusic
```
...
- [compose_install_xiaomusic](../c/compose_install_xiaomusic.html "小米音乐")  👈 当前所在位置
