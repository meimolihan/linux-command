compose_install_openlist
===

自动化部署 Openlist 媒体目录服务的 Linux Docker 部署脚本，可自动配置环境、生成文件、一键启动服务。


## 一键脚本

```bash
bash <(curl -sL gitee.com/meimolihan/linux-command_sh/raw/master/compose_install_openlist.sh) 5244 /vol1/1000/compose/openlist
```

- 安全提示：首次启动后，请立即设置管理员密码：

```bash
docker exec -it openlist ./openlist admin set YOUR_SECURE_PASSWORD
docker restart openlist
```

## 项目简介

**[OpenList](https://doc.openlist.team/ "OpenList 文档")** 是一款功能强大的网盘挂载和管理工具，支持将多种云存储服务（如本地磁盘、WebDAV、SFTP、S3、OneDrive、Google Drive、百度网盘等）统一挂载到一个Web界面中。结合 Docker 部署，您可以在任何支持 Docker 的平台（Linux、Windows、macOS、NAS 等）上快速部署和运行 OpenList，实现多存储服务的集中管理和便捷访问。

  - **🌐 `OpenList` 官网地址：[https://doc.openlist.team/](https://doc.openlist.team/ "OpenList 文档")**
  - **🐱 `GitHub` 项目地址：[https://github.com/OpenListTeam/OpenList](https://github.com/OpenListTeam/OpenList "GitHub 代码托管平台")**
  - **🐋 `Docker` 镜像地址：[https://hub.docker.com/r/openlistteam/openlist](https://hub.docker.com/r/openlistteam/openlist "Docker Hub 镜像地址")**

## 效果预览

![执行脚本效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_openlist-01.webp "截图演示")

![WEB效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_openlist-02.webp "截图演示")

## 补充说明

该脚本用于一键部署 OpenList 媒体目录服务，基于 Docker Compose 实现，适合需要统一管理多种云存储服务的场景。

### 功能特点

* 支持交互式和传参两种部署模式（可指定目录和端口）
* 自动检查并安装 Docker 和 Docker Compose 环境
* 智能端口管理：检查端口 5244 是否放行，自动开放并持久化 iptables 规则
* 清理旧容器：部署前自动清理 openlist 容器和相关镜像
* 自动生成 docker-compose.yml 配置文件
* 使用 bridge 网络模式：容器独立网络栈
* 数据持久化：配置文件存储在 ./config 目录（/opt/openlist/data）
* 以 root 用户运行：确保文件系统权限正常
* 部署后显示容器状态和访问地址
* 首次启动后需手动设置管理员密码
...
| 部署目录 | 显示 Compose 文件存放路径（默认 /vol1/1000/compose/openlist） |
| 映射端口 | 显示服务访问端口（默认 5244） |
| 端口状态 | 检查并开放防火墙端口 5244 |
| 容器清理 | 显示旧容器（openlist）和镜像的清理结果 |
| 配置文件 | 显示 docker-compose.yml 创建状态 |
| 容器启动 | 显示容器启动结果 |
| 容器状态 | 显示容器 ID、名称、状态、端口等信息 |
| 访问地址 | 显示 OpenList 的 HTTP 访问 URL |

### 注意事项

* 脚本需要 root 权限或 sudo 权限来安装依赖和配置防火墙
* 默认使用 openlistteam/openlist:latest 镜像
* 部署目录默认为 /vol1/1000/compose/openlist，可通过参数修改
* 默认端口为 5244，映射容器内的 5244 端口
* 容器以 root 用户（user: 0:0）运行，确保权限正常
* 数据持久化存储在部署目录下的 ./config 目录
* 首次启动后必须设置管理员密码：docker exec -it openlist ./openlist admin set 密码
* 支持多种云存储服务：本地磁盘、WebDAV、SFTP、S3、OneDrive、Google Drive、百度网盘等
...
> 2. 先目录后端口 
>
>   > 脚本.sh /vol1/1000/compose/openlist 5244
> 3. 先端口后目录 
>
>   > 脚本.sh 5244 /vol1/1000/compose/openlist
> 4. 只传目录 
>
>   > 脚本.sh /vol1/1000/compose/openlist
> 5. 只传端口 
>
>   > 脚本.sh 5244
...
docker rm -f openlist && docker rmi -f openlistteam/openlist:latest && rm -rf /vol1/1000/compose/openlist
```
...
- [compose_install_openlist](../c/compose_install_openlist.html "Openlist 网盘挂载")  👈 当前所在位置
