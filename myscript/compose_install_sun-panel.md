...
```bash
bash <(curl -sL gitee.com/meimolihan/linux-command_sh/raw/master/compose_install_sun-panel.sh) 3002 /vol1/1000/compose/sun-panel
```

## 项目简介

**[Sun-Panel](https://doc.sun-panel.top/zh_cn/ "Sun-Panel 中文文档")** 是一款基于 **Vue** 和 **Go** 语言开发的轻量级导航面板，可以作为服务器、NAS导航面板、Homepage或浏览器首页使用。该项目具有界面简洁、资源占用少的特点，支持Docker部署和ARM架构，无需外部数据库，非常适合个人和企业内部使用。

  - **🌐 `MinDoc` 官网地址：[https://doc.sun-panel.top/zh_cn/](https://doc.sun-panel.top/zh_cn/ "Sun-Panel 中文文档")**
  - **🐱 `GitHub` 项目地址：[https://github.com/hslr-s/sun-panel](https://github.com/hslr-s/sun-panel "GitHub 代码托管平台")**
  - **🐋 `Docker` 镜像地址：[https://hub.docker.com/r/hslr/sun-panel](https://hub.docker.com/r/hslr/sun-panel "Docker Hub 镜像地址")**
  - **🌐 `在线演示`: [http://sunpaneldemo.enianteam.com/](http://sunpaneldemo.enianteam.com/)**
...
![执行脚本效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_sun-panel-01.webp "截图演示")

![WEB效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_sun-panel-02.webp "截图演示")

## 补充说明

该脚本用于一键部署 Sun-Panel 导航面板，基于 Docker Compose 实现，适合作为服务器、NAS导航面板、Homepage或浏览器首页使用。
...
* 清理旧容器：部署前自动清理 sun-panel 容器和相关镜像
* 自动生成 docker-compose.yml 配置文件
* 使用 bridge 网络模式：容器独立网络栈
* 配置目录挂载：./conf 用于存储面板配置（/app/conf）
* 挂载 Docker socket：允许面板管理宿主机上的所有容器
* 界面简洁、资源占用少
* 支持 Docker 部署和 ARM 架构
* 无需外部数据库
* 部署后显示容器状态和访问地址
* 默认登录信息：用户名 admin@sun.cc，密码 12345678
...
| 部署目录 | 显示 Compose 文件存放路径（默认 /vol1/1000/compose/sun-panel） |
| 映射端口 | 显示服务访问端口（默认 3002） |
| 端口状态 | 检查并开放防火墙端口 |
| 容器清理 | 显示旧容器（sun-panel）和镜像的清理结果 |
| 配置文件 | 显示 docker-compose.yml 创建状态 |
| 容器启动 | 显示容器启动结果 |
| 容器状态 | 显示容器 ID、名称、状态、端口等信息 |
| 访问地址 | 显示 Sun-Panel 的 HTTP 访问 URL |

### 注意事项

* 脚本需要 root 权限或 sudo 权限来安装依赖和配置防火墙
* 默认使用 hslr/sun-panel:latest 镜像
* 部署目录默认为 /vol1/1000/compose/sun-panel，可通过参数修改
* 默认端口为 3002，映射容器内的 3002 端口
* 配置数据持久化存储在部署目录下的 ./conf 目录
* 挂载 /var/run/docker.sock 允许面板管理宿主机容器
* 默认登录用户名 admin@sun.cc，密码 12345678，首次登录后请修改
* 支持 ARM 架构设备部署
* 无需外部数据库，使用内置数据存储
...
> 2. 先目录后端口 
>
>   > 脚本.sh /vol1/1000/compose/sun-panel 3002
> 3. 先端口后目录 
>
>   > 脚本.sh 5000 /vol1/1000/compose/sun-panel
> 4. 只传目录 
>
>   > 脚本.sh /vol1/1000/compose/sun-panel
> 5. 只传端口 
>
>   > 脚本.sh 3002
...
docker rm -f sun-panel && docker rmi -f hslr/sun-panel:latest && rm -rf /vol1/1000/compose/sun-panel
```
...
- [compose_install_sun-panel](../c/compose_install_sun-panel.html "Sun‑Panel 导航面板")  👈 当前所在位置
