compose_install_dufs
===

Dufs 私有网盘 / 文件共享 / 静态服务器 / WebDAV 盘

## 一键脚本

```bash
bash <(curl -sL gitee.com/meimolihan/linux-command_sh/raw/master/compose_install_dufs.sh) 5000 /vol1/1000/compose/dufs
```

## 项目简介

**[Dufs](https://github.com/sigoden/dufs "GitHub 代码托管平台")** 是一个轻量级、高性能的文件服务器，支持多种文件操作功能。通过 Docker 部署，可以快速搭建个人或团队使用的文件共享服务。🌟

  - **🐱 `GitHub` 原作者项目地址：[https://github.com/sigoden/dufs](https://github.com/sigoden/dufs "GitHub 原作者项目地址")**
  - **🐋 `Docker` 原作者镜像地址：[https://hub.docker.com/r/sigoden/dufs](https://hub.docker.com/r/sigoden/dufs "Docker 原作者镜像地址")**
  - **🐱 `GitHub` 我的项目地址：[https://github.com/meimolihan/dufs-zh](https://github.com/meimolihan/dufs-zh "GitHub 我的项目地址")**
  - **🐋 `Docker` 我的镜像地址：[https://hub.docker.com/repository/docker/mobufan/dufs-zh](https://hub.docker.com/repository/docker/mobufan/dufs-zh "Docker 我的镜像地址")**

## 效果预览

![执行脚本效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_dufs-01.webp "截图演示")

![WEB效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_dufs-02.webp "截图演示")

## 补充说明

该脚本用于一键部署 Dufs 私有网盘/文件共享服务，基于 Docker Compose 实现，适合快速搭建个人或团队的文件服务器。
...
| 访问地址 | 显示服务的 HTTP 访问 URL |

### 注意事项

* 脚本需要 root 权限或 sudo 权限来安装依赖和配置防火墙
* 默认使用 mobufan/dufs-zh:latest 镜像（中文版 Dufs）
* 部署目录默认为 /vol1/1000/compose/dufs，可通过参数修改
* 默认端口为 5000，确保端口未被占用且防火墙已开放
* 数据持久化存储在部署目录下的 ./data 目录
...
> 2. 先目录后端口 
>
>   > 脚本.sh /vol1/1000/compose/dufs 5000
> 3. 先端口后目录 
>
>   > 脚本.sh 5000 /vol1/1000/compose/dufs
> 4. 只传目录 
>
>   > 脚本.sh /vol1/1000/compose/dufs
> 5. 只传端口 
>
>   > 脚本.sh 5000
...
docker rm -f dufs && docker rmi -f mobufan/dufs-zh:latest && rm -rf /vol1/1000/compose/dufs
```
...
- [compose_install_dufs](../c/compose_install_dufs.html "Dufs 文件服务")  👈 当前所在位置
