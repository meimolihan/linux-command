...
批量递归更新指定目录下所有 Docker Compose 项目，自动拉取最新镜像、重新创建容器、清理无用镜像，并显示每个项目的更新状态与统计信息。

## 一键脚本

```bash
bash <(curl -sL gitee.com/meimolihan/linux-command_sh/raw/master/compose_all_update.sh) /vol1/1000/compose
```

## 效果预览

![](https://file.meimolihan.eu.org/screenshot/compose_all_update.webp)

## 补充说明

该脚本用于批量递归更新指定目录下所有 Docker Compose 项目，基于 docker-compose/docker compose 命令实现，适合需要一键更新多个容器项目的场景。

### 功能特点

* 全自动批量更新：递归扫描并拉取最新镜像、重新创建容器
* 智能更新检测：区分镜像更新、容器更新、无变化等情况
* 项目名识别：支持从 docker-compose.yml、.env 中提取项目名称
* 自动清理镜像：更新后执行 docker image prune 清理无用镜像
* 容器状态显示：更新后展示每个项目的容器运行状态
* 详细统计报告：显示总计、成功、失败、有更新、无更新的项目数量
...
* 更新操作会重新创建容器，但保留数据卷
* 默认递归扫描最大深度为 3 级目录
* 支持 docker-compose.yml、docker-compose.yaml、compose.yml 三种文件名
* 更新失败的项目会显示错误输出（最多5行）
* 建议更新后检查容器健康状态，确保服务正常运行
* 脚本会自动检测并使用可用的 compose 命令（docker-compose 或 docker compose）
