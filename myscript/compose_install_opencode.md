compose_install_opencode
===

OpenCode 开源轻量在线代码编辑器 / 网页版代码管理器。

## 一键脚本

```bash
bash <(curl -sL gitee.com/meimolihan/linux-command_sh/raw/master/compose_install_opencode.sh) 3005 /vol1/1000/compose/opencode
```

## 项目简介

 **[OpenCode](https://opencode.hanxi.cc/ "OpenCode 镜像站")** 是一个开源轻量在线代码编辑器，支持网页端直接管理文件、编辑配置与代码。通过 Docker 部署，可快速在 NAS 或服务器搭建轻量化 WebIDE，随时随地修改脚本、容器配置、文档与各类配置文件，简洁高效、占用极低，日常运维编辑十分便捷。 

- **🌐 `OpenCode` 官网地址：[https://opencode.ai](https://opencode.ai "OpenCode 官网")**  
- **🐱 `GitHub` 项目地址：[https://github.com/anomalyco/opencode](https://github.com/anomalyco/opencode "GitHub 项目地址")**  

## 效果预览

![执行脚本效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_opencode-01.webp "截图演示")

![WEB效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_opencode-02.webp "截图演示")

## 补充说明

该脚本用于一键部署 OpenCode 在线代码编辑器，基于 Docker Compose 实现，适合在 NAS 或服务器搭建轻量化 WebIDE。
...
* 支持 TTY 和 stdin 交互：适合交互式编辑操作
* 工作区挂载：./workspace 用于存储编辑的文件
* 配置目录：./config 用于存储 OpenCode 配置
* 部署后显示容器状态和访问地址
...
| 部署目录 | 显示 Compose 文件存放路径（默认 /vol1/1000/compose/opencode） |
| 映射端口 | 显示服务访问端口（默认 3005，映射容器内 3000） |
| 端口状态 | 检查并开放防火墙端口 |
| 容器清理 | 显示旧容器（opencode）和镜像的清理结果 |
| 配置文件 | 显示 docker-compose.yml 创建状态 |
| 容器启动 | 显示容器启动结果 |
| 容器状态 | 显示容器 ID、名称、状态、端口等信息 |
| 访问地址 | 显示 OpenCode 的 HTTP 访问 URL |

### 注意事项

* 脚本需要 root 权限或 sudo 权限来安装依赖和配置防火墙
* 默认使用 ghcr.io/anomalyco/opencode:latest 镜像
* 部署目录默认为 /vol1/1000/compose/opencode，可通过参数修改
* 默认端口为 3005，映射容器内的 3000 端口
* 容器支持 TTY 交互，适合需要终端操作的场景
* 工作区数据持久化存储在部署目录下的 ./workspace 目录
* 配置文件存储在部署目录下的 ./config 目录（/root/.opencode）
* 入口点为 opencode，启动命令为 web 模式
...
> 2. 先目录后端口 
>
>   > 脚本.sh /vol1/1000/compose/opencode 3005
> 3. 先端口后目录 
>
>   > 脚本.sh 5000 /vol1/1000/compose/opencode
> 4. 只传目录 
>
>   > 脚本.sh /vol1/1000/compose/opencode
> 5. 只传端口 
>
>   > 脚本.sh 3005
...
docker rm -f opencode && docker rmi -f ghcr.nju.edu.cn/anomalyco/opencode && rm -rf /vol1/1000/compose/opencode
```
...
- [compose_install_opencode](../c/compose_install_opencode.html "Opencode 在线代码编辑器")  👈 当前所在位置
