compose_install_hd-icons
===

自动化部署高清图标库（hd-icons）Docker 容器的 Linux 脚本，全程可视化、自动配置环境、一键启动服务。

## 一键脚本

```bash
bash <(curl -sL gitee.com/meimolihan/linux-command_sh/raw/master/compose_install_hd-icons.sh) 50560 /vol1/1000/compose/hd-icons
```
...
- **🐱 `GitHub` 项目地址：[https://github.com/xushier/HD-Icons](https://github.com/xushier/HD-Icons "GitHub 代码托管平台")**

## 效果预览

![脚本效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_hd-icons-01.webp "截图演示")

![WEB效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_hd-icons-02.webp "截图演示")

## 补充说明

该脚本用于自动化部署高清图标库（HD-Icons）Docker 容器，基于 Docker Compose 实现，适合开发者和设计师快速搭建图标资源服务。
...
* 支持自定义环境变量：可配置 HTTP_PROXY、HTTPS_PROXY（如需代理）
* 部署后显示容器状态和访问地址
* 图标数据存储：持久化存储在 ./icons 目录
...
| 映射端口 | 显示服务访问端口（默认 50560） |
| 端口状态 | 检查并开放防火墙端口 |
| 容器清理 | 显示旧容器（hd-icons）和镜像的清理结果 |
| 配置文件 | 显示 docker-compose.yml 创建状态 |
| 容器启动 | 显示容器启动结果 |
| 容器状态 | 显示容器 ID、名称、状态、端口等信息 |
| 访问地址 | 显示图标库服务的 HTTP 访问 URL |

### 注意事项

* 脚本需要 root 权限或 sudo 权限来安装依赖和配置防火墙
* 默认使用 xushier/hd-icons:latest 镜像
* 部署目录默认为 /vol1/1000/compose/hd-icons，可通过参数修改
* 默认端口为 50560，确保端口未被占用且防火墙已开放
* 图标数据持久化存储在部署目录下的 ./icons 目录
* 如需使用代理访问，可以取消注释 HTTP_PROXY 和 HTTPS_PROXY 环境变量
* 代理格式：`http://用户名:密码@IP:端口` 或 `http://IP:端口`
...
> 2. 先目录后端口 
>
>   > 脚本.sh /vol1/1000/compose/hd-icons 50560
> 3. 先端口后目录 
>
>   > 脚本.sh 50560 /vol1/1000/compose/hd-icons
> 4. 只传目录 
>
>   > 脚本.sh /vol1/1000/compose/hd-icons
> 5. 只传端口 
>
>   > 脚本.sh 50560
...
docker rm -f hd-icons && docker rmi -f xushier/hd-icons:latest && rm -rf /vol1/1000/compose/hd-icons
```
...
- [compose_install_hd-icons](../c/compose_install_hd-icons.html "HD-Icons 图标库")  👈 当前所在位置
