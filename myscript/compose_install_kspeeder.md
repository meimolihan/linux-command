compose_install_kspeeder
===

KSpeeder 自建私有加速代理优选，全设备共享加速，省心高效。

## 一键脚本

```bash
bash <(curl -sL gitee.com/meimolihan/linux-command_sh/raw/master/compose_install_kspeeder.sh) 5003 /vol1/1000/compose/kspeeder
```

## 🎯 项目简介

**[KSpeeder](https://kspeeder.istoreos.com/ "Kspeeder 项目")** 是一款由 **iStoreOS 团队**开发的智能 Docker 镜像加速器服务。它通过先进的缓存技术和代理机制，显著提升 Docker 镜像的拉取速度，特别针对国内网络环境访问 Docker Hub 等海外镜像仓库进行了深度优化。🚀

  - **🌐 `KSpeeder` 官网地址：<https://kspeeder.istoreos.com/>**
  - **🐱 `GitHub` 项目地址：<https://github.com/linkease/docker_kspeeder>**
  - **🐋 `Docker` 镜像地址：<https://hub.docker.com/r/linkease/kspeeder>**

**使用 KSpeeder**

1. 修改或创建 Docker 配置文件 `/etc/docker/daemon.json`，重启 Docker 服务使配置生效
...
![执行脚本效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_kspeeder-01.webp "截图演示")

![WEB效果预览](https://file.meimolihan.eu.org/screenshot/compose_install_kspeeder-02.webp "截图演示")

## 补充说明

该脚本用于一键部署 KSpeeder 自建私有加速代理，基于 Docker Compose 实现，适合需要加速 Docker 镜像拉取和访问海外服务的场景。

### 功能特点

* 支持交互式和传参两种部署模式（可指定目录和端口）
* 自动检查并安装 Docker 和 Docker Compose 环境
* 智能端口管理：检查端口是否放行，自动开放并持久化 iptables 规则（包括 5443 加速端口）
* 清理旧容器：部署前自动清理同名容器和相关镜像
* 自动生成 docker-compose.yml 配置文件
* 双端口映射：5003 为服务访问端口，5443 为加速代理端口
* 数据持久化：配置和数据存储在 ./kspeeder-data 和 ./kspeeder-conf 目录
* 部署后显示容器状态和访问地址
* 自动配置 Docker Hub 镜像加速（需手动配置 daemon.json）
...
| 映射端口 | 显示服务访问端口（默认 5003）和加速端口（5443） |
| 端口状态 | 检查并开放防火墙端口（TCP + UDP） |
| 容器清理 | 显示旧容器（kspeeder）和镜像的清理结果 |
| 配置文件 | 显示 docker-compose.yml 创建状态 |
| 容器启动 | 显示容器启动结果 |
| 容器状态 | 显示容器 ID、名称、状态、端口等信息 |
| 访问地址 | 显示 KSpeeder 的 HTTP 访问 URL |

### 注意事项

* 脚本需要 root 权限或 sudo 权限来安装依赖和配置防火墙
* 默认使用 linkease/kspeeder:latest 镜像
* 部署目录默认为 /vol1/1000/compose/kspeeder，可通过参数修改
* 默认服务端口为 5003，加速代理端口为 5443
* 如需配置 Docker Hub 加速，需手动修改 /etc/docker/daemon.json 并重启 Docker
* 配置示例：`{"registry-mirrors": ["https://registry.linkease.net:5443"]}`
* 数据持久化存储在部署目录下的 ./kspeeder-data 和 ./kspeeder-conf 目录
* 脚本会自动安装 iptables-persistent 或 iptables-services 来持久化防火墙规则
* UDP 端口也会开放，用于某些特殊的加速场景
...
> 2. 先目录后端口 
>
>   > 脚本.sh /vol1/1000/compose/kspeeder 5003
> 3. 先端口后目录 
>
>   > 脚本.sh 5003 /vol1/1000/compose/kspeeder
> 4. 只传目录 
>
>   > 脚本.sh /vol1/1000/compose/kspeeder
> 5. 只传端口 
>
>   > 脚本.sh 5003
...
docker rm -f kspeeder && docker rmi -f linkease/kspeeder:latest && rm -rf /vol1/1000/compose/kspeeder
```
...
- [compose_install_kspeeder](../c/compose_install_kspeeder.html "Kspeeder 加速器")  👈 当前所在位置
